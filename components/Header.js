...
const Header = () => {
    ...
    const [currentWiki, setCurrentWiki] = useState(config.SUPPOST_WIKI[0].PAEAM);

    useEffect(() => {
        const wikiParam = searchParams.get('wiki');
        if (wikiParam) setCurrentWiki(wikiParam);
    }, [searchParams]);
        ...
        <header class="relative bg-gray-800/50 after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/10">
            <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                <div class="relative flex h-16 items-center justify-between">
                    ...
                    <button type="button" class="flex items-center gap-1 rounded-full bg-white/5 px-3 py-2 text-gray-300 hover:text-white" aria-label="Search">
                        <span class="sr-only">搜索</span>
                        <svg width="15" height="20" class="Search-Icon" viewBox="0 0 20 20" aria-hidden="true">
                            ...
                            <el-options anchor="bottom start" popover class="max-h-56 w-4xs overflow-auto rounded-md bg-gray-800 py-1 text-base [--anchor-gap:--spacing(1)] data-leave:transition data-leave:transition-discrete data-leave:duration-100 data-leave:ease-in data-closed:data-leave:opacity-0 sm:text-sm">
                                {config.SUPPOST_WIKI.map((option, index) => (
                                    <el-option value={option.PAEAM} class="group/option relative block cursor-default py-2 pr-9 pl-3 text-sm font-medium text-gray-300 select-none focus:text-white">
                                        <a href={`?wiki=${option.PAEAM}`} class="flex items-center cursor-pointer">
